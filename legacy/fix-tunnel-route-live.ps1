...
# Moves quant-live.carsonng.com from native's now-decommissioned port 8081 to the new
# Docker live deployment's port 18081 -- mirrors fix-tunnel-route.ps1's own paper cutover
# (quant.carsonng.com: 8080 -> 18080) exactly, same tunnel/account.
$newIngress = $current.result.config.ingress | ForEach-Object {
    if ($_.hostname -eq "quant-live.carsonng.com") {
        [PSCustomObject]@{ hostname = $_.hostname; service = "http://localhost:18081" }
    } else { $_ }
}
...
try {
    $check = Invoke-WebRequest -Uri "https://quant-live.carsonng.com/" -UseBasicParsing -TimeoutSec 10
    Write-Host "https://quant-live.carsonng.com/ -> HTTP $($check.StatusCode)"
