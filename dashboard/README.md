# 实时交易分析看板（决策支持 + 模拟自动执行）

NiceGUI 看板：31 个品种（金属/能源/外汇/指数/加密）的实时分析 + 模拟盘自动执行（MT5 demo）。

> **策略已转为「周线趋势」(2026-06)**：日线技术策略经全面 OOS 验证后**无可证明优势**
> （Deflated Sharpe 53–58%）；唯一经验证的边际是**周线时间序列动量**（OOS +0.11R，DSR 100%，
> ~+3% 年化/0.5% 风险），且只在**商品（金属）+ 股指**有效，外汇无效（已剔除）。详见根 `README.md`。
> 信号在周线 bar 上生成（`providers.get_history` 取 W1），持仓 ~7 周（`HORIZON_CAL=49`），
> 每 1–2 周约 1 单。关键开关在 `paper.py`：`MIN_STRENGTH`、`OVEREXT_FILTER`、
> `WEEKLY_TREND_CLASSES`、`RISK_PER_TRADE`。客观闸门模型（`confidence_model`/`win_model`）
> 已用周线数据重建。
...
- **顶部**：自动刷新选择器（1 / 10 / 15 / 30 / 60 分钟，默认 **10**）、周末暂停 LLM 开关、
...
- **LLM 板扫描**：**一次调用分析全部品种**（批量），最短间隔 10 分钟，且受预算守卫保护。
  → 默认 10 分钟约 144 次/天，仍低于 200 上限，留有手动深挖的余量。
...
3. 发现你经纪商的 Gold/Oil 符号名（各家不同）：
   ```powershell
   uv run python -m dashboard.data.mt5_client
   ```
...
→ LLM 置信度≥0.60 → 趋势强度=5/5 → 通过风控（有效 ATR 止损）→ R:R≥1.5
→ **入场用实时价**（非滞后 K 线收盘，避免开仓即被打止损的 bug）→ 同品种无未平仓
→ **冷却**（同品种平仓后 60 分钟内不再开）→ **去相关**（每个美元方向只持一个品种）。
每个被拒的信号都记录原因，不静默跳过。

**两种 SL/TP 并行对比**：
- `ATR`：SL=1.5×ATR，TP 按固定 R:R（默认 3:1，见下方配置说明）。
...
**历史回放（加速器）**：`python -m dashboard.research.replay --period 5y` —— 用**仅确定性**信号
...
## 当前配置 + 前向测试协议（关键：测试期间勿调参）

### 当前配置（由 `optimize.py` 在样本内选出）
`paper.py` 里的参数：

| 参数 | 值 | 说明 |
|------|----|------|
| `RR_DEFAULT` | **3.0** | 止盈:止损 = 3:1（只需 ~25% 胜率即盈亏平衡） |
| `SL_ATR_MULT` | **1.5** | 止损 = 1.5×ATR |
| `MIN_STRENGTH` | **5** | 只接受 5/5 满分趋势对齐（交易很少、质量高） |
| `COOLDOWN_MIN` | 60 | 同品种平仓后冷却 60 分钟 |
| `DECORRELATE` | True | 每个美元方向只持一个品种 |
| `RISK_PER_TRADE` | 0.005 | 每笔风险 0.5% 名义本金（仅用于计算手数） |

### 这套配置的真实状态：**未经证实的假设，不是已验证的优势**
- 复现命令：`python -m dashboard.research.optimize --period 5y`
- 结果：在 45 组参数中按样本内期望选出，**样本外（最后 40%）期望 +0.32 R，但 Deflated Sharpe 仅 43%**（远低于 95% 的门槛）。
- 含义：**有约 43% 的概率这个优势连"为正"都不成立**；样本外的正收益很可能是最近两年的趋势行情（regime luck），换成震荡行情会被抹平。
- **结论：可以前向验证，但现在不要投入真实资金。**

### 前向测试协议（pre-registered）
1. **冻结参数。** 上表已是预先声明的配置。**测试期间不要再改任何参数**（改了就是在历史数据上过拟合，DSR 会被多重检验惩罚，结果失去意义）。
2. **让它自动跑**，不要手动挑单、不要跳过亏损单（破坏样本无偏性）。
3. **判定门槛（预先设定，不许中途移动）**：累计 **≥30 笔去相关后的已平仓交易**，且
   - 期望值 expectancy > 0 **且**
   - Deflated Sharpe ≥ 95%
   才算"可能有真优势"。
4. **达标后**：才考虑用极小真实手数继续验证。**未达标 / DSR 回落**：判定为 regime luck，放弃方向性交易，把系统作为纯研究/监控工具。
5. **绝不要**因为"最近一周/几笔很好看"就加大仓位——那是方差，不是技能。这正是无优势系统骗人的方式。

> 一句话：现在的配置是"目前能honestly找到的最佳估计"，但 DSR=43% 说明它还没被证明。**唯一诚实的下一步是冻结参数、前向跑、用累计 DSR 判定**——而不是回头再调参。
